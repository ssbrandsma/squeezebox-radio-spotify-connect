    ...
    async fn load_remote_track(
        ...
        loop {
            ...
            let (offset, mut normalisation_data) = if is_ogg_vorbis {
                #[cfg(feature = "streaming-only")]
                if self.config.passthrough {
                    (SPOTIFY_OGG_HEADER_END, None)
                } else {
                ...
                #[cfg(not(feature = "streaming-only"))]
                {
            ...
            #[cfg(feature = "streaming-only")]
            let audio_file = if self.config.passthrough {
                SkipReader::new(decrypted_file, offset as usize)
            } else {
                return None;
            };
            #[cfg(not(feature = "streaming-only"))]
            ...
            #[cfg(all(feature = "passthrough-decoder", feature = "streaming-only"))]
            let decoder_type = PassthroughDecoder::new(audio_file, format).map(|x| Box::new(x) as Decoder);
            ...
            let position_ms = if cfg!(feature = "streaming-only") {
                if position_ms != 0 { warn!("streaming-only mode ignores requested start position {position_ms} ms"); }
                0
            } else if position_ms > duration_ms {
                warn!(
...
#[cfg(feature = "streaming-only")]
struct SkipReader<T: Read + Seek> {
    inner: T,
    remaining: usize,
}

#[cfg(feature = "streaming-only")]
impl<T: Read + Seek> SkipReader<T> {
    fn new(inner: T, remaining: usize) -> Self { Self { inner, remaining } }
}

#[cfg(feature = "streaming-only")]
impl<T: Read + Seek> Read for SkipReader<T> {
    fn read(&mut self, out: &mut [u8]) -> io::Result<usize> {
        while self.remaining > 0 {
            let mut scratch = [0u8; 256];
            let limit = self.remaining.min(scratch.len());
            let n = self.inner.read(&mut scratch[..limit])?;
            if n == 0 { return Ok(0); }
            self.remaining -= n;
        }
        self.inner.read(out)
...
#[cfg(feature = "streaming-only")]
impl<T: Read + Seek> Seek for SkipReader<T> {
    fn seek(&mut self, pos: SeekFrom) -> io::Result<u64> {
        if matches!(pos, SeekFrom::Current(0)) { Ok(0) }
        else { Err(io::Error::new(io::ErrorKind::Unsupported, "streaming-only mode does not support seek")) }
    }
}

#[cfg(feature = "streaming-only")]
impl<T: Read + Seek + Send + Sync> MediaSource for SkipReader<T> {
