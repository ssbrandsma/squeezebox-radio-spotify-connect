...
pub enum AudioFile {
    ...
    #[cfg(feature = "streaming-only")]
    Sequential(streaming::SequentialAudio),
}
...
impl AudioFile {
    pub async fn open(
        ...
        #[cfg(feature = "streaming-only")]
        return Ok(AudioFile::Sequential(streaming::SequentialAudio::open(session.clone(), file_id).await?));
    ...
    pub fn get_stream_loader_controller(&self) -> Result<StreamLoaderController, Error> {
        let controller = match self {
            ...
            #[cfg(feature = "streaming-only")]
            AudioFile::Sequential(stream) => StreamLoaderController {
                channel_tx: None,
                stream_shared: None,
...
impl Read for AudioFile {
    fn read(&mut self, output: &mut [u8]) -> io::Result<usize> {
        match *self {
            ...
            #[cfg(feature = "streaming-only")]
            AudioFile::Sequential(ref mut file) => file.read(output),
        }
    }
}
...
impl Seek for AudioFile {
    fn seek(&mut self, pos: SeekFrom) -> io::Result<u64> {
        match *self {
            ...
            #[cfg(feature = "streaming-only")]
            AudioFile::Sequential(ref mut file) => file.seek(pos),
        }
    }
}
