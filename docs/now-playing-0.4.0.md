...
screen updates text immediately and starts one artwork lookup. Artist and title
use separate full-width rows at the top of the 320x240 display. The artwork is
centered in the remaining lower area, with framework overlays hidden so the
clock cannot obscure the metadata. Artwork first
...
512 KiB, decoded from memory, scaled to the 168-pixel artwork area, and the
...
3. Check that artist and title span the top of the screen and that cover art is
   centered below them.
